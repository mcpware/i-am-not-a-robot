...
/**
 * i-am-not-a-robot — the missing input() for headless agents.
 *
 ...
 * (toggle with { log: false } or IAMNOTAROBOT_QUIET=1).
 */
...
async function captureScreenshot(page, capture, log) {
  if (!page || typeof page.screenshot !== 'function') {
    throw new Error(
      'i-am-not-a-robot: first arg must be a page-like object with a screenshot() method ' +
...
<h2 style="margin:0 0 4px">i-am-not-a-robot</h2>
...
async function humanGate(page, opts = {}) {
  ...
  const timer = setTimeout(() => {
    ...
    rejectAnswer(new Error(`i-am-not-a-robot: timed out after ${timeoutMs}ms waiting for a human answer`));
  }, timeoutMs);
