...
/**
 * i-am-not-a-robot MCP server (stdio).
 *
...
const server = new McpServer({ name: 'i-am-not-a-robot', version: '0.1.0' });
...
async function main() {
  ...
  if (process.env.IAMNOTAROBOT_QUIET !== '1') {
    console.error('[i-am-not-a-robot] MCP server ready (stdio) — tools: start_human_relay, await_human_solve');
  }
}
...
main().catch((e) => {
  console.error('[i-am-not-a-robot] fatal:', e);
  process.exit(1);
});
